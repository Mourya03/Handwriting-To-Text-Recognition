...
def write_summary(char_error_rates: List[float], word_accuracies: List[float]) -> None:
    ...
    with open(FilePaths.fn_summary, 'w') as f:
        json.dump({'charErrorRates': char_error_rates,
...
def infer(model: Model, fn_img: Path) -> None:
    ...
    win.title("Handwriting Text Recognizer")

    Label(win, text="HandWriting Text Recognition Engine",
          font='Arial 17 bold').pack(pady=15)

   
    image_path = filedialog.askopenfilename(title="Open an Image", filetypes=(("All Files","*.*"),("PNG", "*.png*"), ("JPG", "*.jpg"),("JPEG","*.jpeg")))
    ...
    Label(win, text=f'Recognized Text: {recognized[0]}',font='Arial 14 bold').pack(pady=7)
    Label(win, text=f'Probability: {probability[0]}',font='Arial 14').pack(pady=7)
    ...
    Button(win, text="Choose Another Image", command=restart, font="Arial 10 bold").pack(pady=2)
    Button(win, text="Exit", command=exit, font="Arial 10 bold").pack(pady=2)
...
while infer(model, args.img_file):
    pass